...
export async function syncNoncesFromChain(
  ...
  for (const user of users) {
    try {
      // Sync Merkle withdrawal nonces (always available on SettlementV2)
      const merkleNonce = await publicClient.readContract({
      ...
      // Sync fast withdrawal nonces (may not exist on SettlementV2 — optional)
      let fastNonce = 0n;
      try {
        fastNonce = await publicClient.readContract({
          address: state.contractAddress,
          abi: NONCE_ABI,
        ...
        // fastWithdrawalNonces not available on this contract — skip silently
      }
...
/**
 * Reset a single user's nonce from on-chain state.
 * Called by reconcilePendingWithdrawals() when a withdrawal tx reverts —
 * prevents nonce drift that causes InvalidSignature on subsequent withdrawals.
 *
 * Pattern: dYdX v3 ForcedActionRegistry — chain state is authoritative,
 * engine state must re-sync after failed tx.
 */
export async function resetNonceFromChain(
  publicClient: PublicClient,
  user: Address,
): Promise<{ merkleNonce: bigint; fastNonce: bigint } | null> {
  ...
  try {
    ...
    try {
      ...
      // fastWithdrawalNonces not available on this contract
    }

    const key = user.toLowerCase();
    const oldMerkle = state.nonces.get(key) ?? 0n;
    const oldFast = state.fastNonces.get(key) ?? 0n;

    state.nonces.set(key, merkleNonce);
    state.fastNonces.set(key, fastNonce);

    if (oldMerkle !== merkleNonce || oldFast !== fastNonce) {
      console.log(`[Withdraw] Nonce reset for ${user.slice(0, 10)}: merkle ${oldMerkle}→${merkleNonce}, fast ${oldFast}→${fastNonce}`);
    }

    return { merkleNonce, fastNonce };
  } catch (e) {
    console.error(`[Withdraw] Failed to reset nonce for ${user.slice(0, 10)}:`, e);
    return null;
  }
}
